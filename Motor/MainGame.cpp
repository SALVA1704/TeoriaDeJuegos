...
void MainGame::initLevel() {
	...
	// Zombies
	const auto& zombiePositions = levels[currentLevel]->getZombiesPosition();
	for (const auto& position : zombiePositions) {
		zombies.emplace_back(new Zombie());
		zombies.back()->init(1.5f, position);
	}
}
